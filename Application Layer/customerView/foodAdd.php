    ...
    switch ($_GET["action"]) {
        case "add":
            if (!empty($_REQUEST["quantity"])) {
                ...
                /*
                implementing concept of php arrays for inserting into "cart_item2" variable which stores
                values related to order
                */
                $productByCode = runQuery("select * from food where foodName='$foodName'");
                $itemArray = array($productByCode[0]["foodName"] => array('foodName' => $productByCode[0]["foodName"], 'photo' => $$productByCode[0]["photo"], 'price' => $productByCode[0]["price"], 'quantity' => $_REQUEST["quantity"]));
                if (!empty($_SESSION["cart_item2"])) {
                    if (in_array($productByCode[0]["foodName"], $_SESSION["cart_item2"])) {
                        foreach ($_SESSION["cart_item2"] as $k => $v) {
                            if ($productByCode[0]["foodName"] == $k)
                                $_SESSION["cart_item2"][$k]["quantity"] = $_REQUEST["quantity"];
                        }
                    }
                    else {
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li><a href="../customerView/foodView.php">View</a></li>
...
<body>
  ...
  <?php
  $query1="select * from food order by foodName asc";
      ...
      <div class="well">
          ...
          <tbody>
            ...
            while($row=mysqli_fetch_array($result1)) {
                ?>
                <tr>
                    <form action="foodAdd.php" method="get">
