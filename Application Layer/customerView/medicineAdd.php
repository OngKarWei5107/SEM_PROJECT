    ...
    switch ($_GET["action"]) {
        case "add":
            if (!empty($_REQUEST["quantity"])) {
                ...
                */
                $productByCode = runQuery("select * from medicine where medicineName='$medicineName'");
                $itemArray = array($productByCode[0]["medicineName"] => array('medicineName' => $productByCode[0]["medicineName"], 'photo' => $$productByCode[0]["photo"], 'price' => $productByCode[0]["price"], 'quantity' => $_REQUEST["quantity"]));
                if (!empty($_SESSION["cart_item1"])) {
                    if (in_array($productByCode[0]["medicineName"], $_SESSION["cart_item1"])) {
                        foreach ($_SESSION["cart_item1"] as $k => $v) {
                            if ($productByCode[0]["medicineName"] == $k)
                                $_SESSION["cart_item1"][$k]["quantity"] = $_REQUEST["quantity"];
                        }
                    }
                    else {
...
<body>
  ...
  <?php
  $query1="select * from medicine order by medicineName asc";
      ...
      <div class="well">
          ...
          <tbody>
            ...
            while($row=mysqli_fetch_array($result1)) {
                ?>
                <tr>
                    <form action="medicineAdd.php" method="get">
