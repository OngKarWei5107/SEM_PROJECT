    ...
    switch ($_GET["action"]) {
        case "add":
            if (!empty($_REQUEST["quantity"])) {
                $petItemName = $_REQUEST['petItemName'];
                ...
                */
                $productByCode = runQuery("select * from petItem where petItemName='$petItemName'");
                $itemArray = array($productByCode[0]["petItemName"] => array('petItemName' => $productByCode[0]["petItemName"], 'photo' => $productByCode[0]["photo"], 'price' => $productByCode[0]["price"], 'quantity' => $_REQUEST["quantity"]));
                if (!empty($_SESSION["cart_item"])) {
                    if (in_array($productByCode[0]["petItemName"], $_SESSION["cart_item"])) {
                        foreach ($_SESSION["cart_item"] as $k => $v) {
                            if ($productByCode[0]["petItemName"] == $k)
            ...
            header("location: petItemAdd.php");
            break;
    }
}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li><a href="../customerView/petItemView.php">View</a></li>
...
<body>
  ...
  <?php
  $query1="select * from petItem order by petItemName asc";
    ...
    <div class="well">
      <div class="panel panel-primary">
        <div class="panel-heading">
          <center><h2>Product Available</h2></center>
        </div>
      </div>
      <div class="well">
          ...
          <tbody>
            ...
            while($row=mysqli_fetch_array($result1)) {
                ?>
                <tr>
                    <form action="petItemAdd.php" method="get">
                        <td><?php echo $count."." ?></td>
                        <td><img src="../serviceProviderView/photo/<?=$row['photo']?>"></td>
                        <td><?php echo $row['petItemName'] ?></td>
                        <td><?php echo $row['price'] ?></td>
                        <td><input type="number" name="quantity" class="col-xs-3"></td>
                        <td>
                          <button type="submit" class="btn btn-primary btn-md">
                          <span class="glyphicon glyphicon-plus"></span></button>
                        </td>
                        <input type="hidden" name="petItemName" value="<?php echo $row['petItemName'] ?>">
    ...
    <div class="container">
      <div class="row">
        <div class="col-md-12 text-center">
          <p>Copyright Company Delivery2U © 2020. All rights reserved.</p>
        </div>
      </div>
    </div>
  </div>
