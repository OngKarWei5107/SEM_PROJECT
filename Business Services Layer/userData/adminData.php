...
* Base URL
*/
define('BASE_URL', '..\..\Business Services Layer\userController\RegisAdmin.php');
...
class adminData
{
	...
	*/
	...
	{
        $query = "SELECT username, psw FROM admin";

        $adminData = DB::run($query)->fetchAll(PDO::FETCH_ASSOC);
	...
	public static function All()
	{
		...
		try {
	    	...
	    	if ($stmt = DB::run($query)) { // this will run the build query
				...
				$adminData = $stmt->fetchAll(PDO::FETCH_ASSOC); // need to add fetchAll for pdo 
	...
	*/
	public static function getById($adminid)
	{
		...
		$param = [':adminid' => $adminid]; // the parameter that will be bind by pdo
		try {
			// use static method run() from class DB 
	    	if ($stmt = DB::Run($query, $param)) { // this will run the build query
				// need to use fetch to retrieve only 1 row of data
				$adminData = $stmt->fetch(PDO::FETCH_ASSOC); // this will retrieve the row of data
	...
	{
		$query = "INSERT INTO admin (`username`, `psw`) VALUES (:username, :psw)";
